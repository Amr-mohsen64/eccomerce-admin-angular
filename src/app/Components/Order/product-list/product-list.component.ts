...
export class ProductListComponent implements OnInit, OnChanges {
  ...
  constructor(private staticPrdService: StaticProductsService
    , private prodService: ProductsService  //new daynamic
  ...
  ngOnChanges(): void {
    ...
    this.prodService.getProductByCatId(this.sentCatID)
      .subscribe(prods => { this.prdListOfCat = prods })
  }
  ...
  deleteProduct(id: number) {
    this.prodService.deletProduct(id).subscribe((product) => {
      alert('Delet Product'+ id)
      this.prdListOfCat = this.prdListOfCat.filter(item => item.id != id);
    });
  }
}
